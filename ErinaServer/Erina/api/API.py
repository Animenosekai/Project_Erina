    ...
    if not ServerConfig.public_api:
        if "key" not in request.values:
            ...
            if not isfile(erina_dir + "/ErinaServer/Erina/auth/apiAuth/" + currentKey + ".erina"):
                return makeResponse(request_args=request.values, cooldown=None, code=400, error="WRONG_KEY", error_message="The given key isn't registered")
            else:
                ...
                if currentKey in rate_limiting_api_map:
                    ...
                    if rate > currentAuth.rate_limit:
                        ...
                        return makeResponse(request_args=request.values, cooldown=currentRateLimit - rate, code=429, error="RATE_LIMITED", error_message="You have exceeded your rate limit")
                else:
                    rate_limiting_api_map[currentKey] = time()
