...
#### REPLIES NEED TO BE LES THAN 280 CHARACTERS
...
def search_anime_by_imageurl(image_url):
    ...
    © Anime no Sekai - 2020
    """
    result = erinasearch.search_anime_by_imageurl(image_url, anilist_priority=config.twitter_anilist_priority)
    if result['erina_cache'] or result['erina_database']:
        return {'similarity': result['similarity'], 'reply': create_reply_for_erinadb(result)}
    elif result['tracemoe_cache'] or result['tracemoe_api']:
        return {'similarity': result['similarity'], 'reply': create_reply_for_tracemoe(result)}
    elif result['saucenao_cache'] or result['saucenao_api']:
        return {'similarity': result['similarity'], 'reply': create_reply_for_saucenao(result)}
    elif result['iqdb_cache'] or result['iqdb_api']:
        return {'similarity': result['similarity'], 'reply': create_reply_for_iqdb(result)}
    else:
...
######## MAKING THE REPLY
    ...
    if anime_info['provider'] == 'anilist':
        length_of_description = 280 - len(reply_text) - 3
        description = anime_info['description']
        reply_text += description[:length_of_description] + '...'
...
def create_reply_for_iqdb(result):
    ...
    if sauce_info['database'] == 'danbooru':
        ...
        reply_text = reply_text[:278 - len(str(sauce_info['url']))]
