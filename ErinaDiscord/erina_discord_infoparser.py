"""
Anime Database Information Parser (for Discord) for the Erina Project

@author: Anime no Sekai
Erina Project - 2020
"""

#### REPLIES NEED TO BE LES THAN 1024 CHARACTERS
...
def search_anime_by_title(query):
    """
    Searching Anime Information by from the title

    Erina Project\n
    © Anime no Sekai - 2020
    """
    result = erinasearch.search_anime_by_title(query, anilist_priority=config.discord_anilist_priority)
    anime_info = {}
    ...
    if result['found_from_anilist_cache'] or result['found_from_anilist_api']:
        ...
        for studio in result['result']['studios']:
            if str(studio['is_animation_studio']):
...
######## MAIN FUNCTION ########
def search_anime_by_imageurl(image_url):
    """
    ...
    © Anime no Sekai - 2020
    """
    result = erinasearch.search_anime_by_imageurl(image_url, anilist_priority=config.discord_anilist_priority)
...
######## MAKING THE REPLY
    ...
    reply_text += ( f"**Episode:** {anime_info['episode']}/{anime_info['episodes']} (at around {anime_info['at']})\n"
    ...
    if anime_info['provider'] == 'anilist':
        if len(anime_info['description']) > 200:
            reply_text += '\n' + anime_info['description'][:200] + '...\n'
        else:
            reply_text += '\n' + anime_info['description'] + '\n'

    reply_text += f"{anime_info['anilist_url']}\n"

    return {'reply': reply_text, 'is_anime': True, 'provider': anime_info['provider'], 'image': anime_info['image'], 'anime': anime_info['anime']}
...
##### CREATING AND FORMATTING THE REPLY
    ######## MAKING THE REPLY
        ...
        sauce_info['thumbnail'] = ''
        if float(result['saucenao_result']['thumbnail'].split('&exp=')[1]) < float(time.time()):
            sauce_info['thumbnail'] = result['saucenao_result']['thumbnail']

        reply_text = ''
        if result['low_similarity']:
            reply_text += 'The similarities seem pretty low but...\n'
        reply_text += (  f"Here is the sauce!\n\n"


                        f"I found it on {str(sauce_info['database'])} (Similarity: {str(sauce_info['similarity'])}%):\n"
        ...
        return {'reply': reply_text, 'is_anime': False, 'image': sauce_info['thumbnail'], 'anime': sauce_info['title']}
...
def create_reply_for_iqdb(result):
    ...
    if result['low_similarity']:
        reply_text += 'The similarities seem pretty low but...\n'
    reply_text += ( f"Here is the sauce!\n"
                    f"I found it on {str(iqdb_api.db_to_name(sauce_info['database']))} (Similarity: {str(sauce_info['similarity'])}%):\n\n"
    )
    ...
    if sauce_info['database'] == 'danbooru':
        ...
        reply_text = reply_text[:1020 - len(str(sauce_info['url']))]
        ...
        try:
            if len(sauce_info['anime_pictures_results']['artist_links']) > 1:
                if len(sauce_info['anime_pictures_results']['artist_links']) == 1:
                    reply_text += '**Artist Link**\n'
                else:
                    reply_text += '**Artist Links**\n'
    ...
    return {'reply': reply_text, 'is_anime': False, 'anime': f'an image from {str(iqdb_api.db_to_name(sauce_info["database"]))}'}